package com.the_changer.touchstonedst.world;
...
public class TouchStoneGenerator {
    public static final int OFFSET_Y = 90;
    static final Identifier TOP_TEMPLATE = new Identifier("touch_stone_structure");
    private static final Identifier MIDDLE_TEMPLATE = new Identifier("touch_stone_structure");
    private static final Identifier BOTTOM_TEMPLATE = new Identifier("touch_stone_structure");
    static final Map<Identifier, BlockPos> OFFSETS;
    static final Map<Identifier, BlockPos> OFFSETS_FROM_TOP;

    public TouchStoneGenerator() {
    }
    ...
    public static void addPieces(StructureManager manager, BlockPos pos, BlockRotation rotation, StructurePiecesHolder holder, Random random) {
        if (random.nextDouble() < 0.5D) {
            ...
            holder.addPiece(new TouchStoneGenerator.Piece(manager, BOTTOM_TEMPLATE, pos, rotation, i * 3));

            for(int j = 0; j < i - 1; ++j) {
                holder.addPiece(new TouchStoneGenerator.Piece(manager, MIDDLE_TEMPLATE, pos, rotation, j * 3));
            }
        }

        holder.addPiece(new TouchStoneGenerator.Piece(manager, TOP_TEMPLATE, pos, rotation, 0));
    }
    ...
    public static class Piece extends SimpleStructurePiece {
        ...
        public Piece(StructureContext structureContext, NbtCompound nbtCompound) {
            super(StructurePieceType.IGLOO, nbtCompound, structureContext.structureManager(), (identifier) -> {
        ...
        private static StructurePlacementData createPlacementData(BlockRotation rotation, Identifier identifier) {
            return (new StructurePlacementData()).setRotation(rotation).setMirror(BlockMirror.NONE).setPosition((BlockPos)TouchStoneGenerator.OFFSETS.get(identifier)).addProcessor(BlockIgnoreStructureProcessor.IGNORE_STRUCTURE_BLOCKS);
        }

        private static BlockPos getPosOffset(Identifier identifier, BlockPos pos, int yOffset) {
            return pos.add((Vec3i)TouchStoneGenerator.OFFSETS_FROM_TOP.get(identifier)).down(yOffset);
        }
        ...
        public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pos) {
            ...
            BlockPos blockPos = (BlockPos)TouchStoneGenerator.OFFSETS_FROM_TOP.get(identifier);
            ...
            if (identifier.equals(TouchStoneGenerator.TOP_TEMPLATE)) {
